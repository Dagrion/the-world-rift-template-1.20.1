    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            ...
            if (charge != null && charge.getClaymore() >= WeaponChargeComponent.MAX) {
                ...
                // Get all entities along the slash path
                ...
                for (LivingEntity target : targets) {
                    ...
                    // Check entity is in front (not behind)
                    double dot = look.dotProduct(directionToTarget);

                    if (dot > 0.5) { // 0.5 = about 60° frontal cone

                        target.damage(
                                serverWorld.getDamageSources().playerAttack(user),
                                BASE_DAMAGE
                        );

                        // Optional knockback in slash direction
                        target.setVelocity(
                                look.x * 1,
                                look.y * 1,
                                look.z * 1
