...
public class GlaiveItem extends SwordItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        WeaponChargeComponent.IncrementGLAIVE(2);
        return true;
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            Vec3d lookingDirection = user.getRotationVec(1.0f);
                ...
                if (WeaponChargeComponent.GLAIVE >= WeaponChargeComponent.MAX_GLAIVE) {
                    WeaponChargeComponent.UseGLAIVE(100);
                    user.setVelocity(
    ...
    public int getItemBarStep(ItemStack stack) {
        return Math.round((float) WeaponChargeComponent.GLAIVE / WeaponChargeComponent.MAX_GLAIVE * 13); // full bar = max charge
    }
    @Override
