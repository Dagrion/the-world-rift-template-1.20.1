...
public class KnifeItem extends SwordItem {
    ...
    public KnifeItem(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
        ...
        builder.put(ReachEntityAttributes.ATTACK_RANGE, new EntityAttributeModifier(ATTACK_REACH_MODIFIER_ID, "Weapon modifier", -3.0, EntityAttributeModifier.Operation.ADDITION));
        ...
        // Register events for invisibility removal
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        WeaponChargeComponent.IncrementKNIFE(2);
        return true;
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            if (WeaponChargeComponent.KNIFE >= WeaponChargeComponent.MAX_KNIFE) {
                WeaponChargeComponent.UseKNIFE(100);
    ...
    public int getItemBarStep(ItemStack stack) {
        return Math.round((float) WeaponChargeComponent.KNIFE / WeaponChargeComponent.MAX_KNIFE * 13);
    }
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (!world.isClient && entity instanceof PlayerEntity player) {
            // Remove invisibility if weapon is not held in main hand or not in the inventory
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                if (!player.getInventory().contains(new ItemStack(this))) {
                    player.removeStatusEffect(ModEffect.INVISIBILITY_KNIFE);
                }
            }
        });
    ...
    private void registerInvisibilityEvents() {
